...
const doc: DocPage = {
  slug: "rules/deterministic-checks",
  title: "Deterministic Checks",
  description:
    "Full reference for the checks: schema. Tagged union of file, regex, ast, diff-evidence, forbidden-import, command, analyzer, agent-process check types.",
  content: `## Deterministic checks
...
The full TypeScript types live in \`packages/engine/src/checks/types.ts\`. This page is the user-facing reference.
...
A \`VIOLATED\` or \`ERROR\` result with \`severity: error\` is blocking. \`warning\` / \`info\` are non-blocking. Advisory sources (\`keyword\`, \`semantic\`, \`llm\`) are not part of this deterministic schema and do not block by default.
...
They can also live in fenced \`rulebound\` blocks when the human-readable body needs to introduce the rule first:
...
Details: [Analyzer Orchestration](/docs/recipes/orchestration).
...
\`scenario\` is not implemented yet. Do not put it in production rules until the engine supports it and the examples are covered by tests.
...
See [Scenario Evidence](/docs/workflows/scenario-evidence) for the full design and the v0.1 product boundary.
...
Strict schema validation fails closed. These examples are invalid and produce actionable config errors instead of silently passing.
...
- [Report Schema](/docs/rules/report-schema) — the JSON shape every check produces and every formatter consumes.
- [Waivers](/docs/rules/waivers) — time-boxed downgrades of deterministic violations.
- [Analyzer Orchestration](/docs/recipes/orchestration) — \`type: analyzer\` in depth.
`,
}
