...
const doc: DocPage = {
  slug: "rules/report-schema",
  title: "Report Schema",
  description:
    "DeterministicReport — the canonical contract returned by validateDeterministic(). Every CLI format, the MCP tools, and downstream dashboards read from this shape.",
  content: `## DeterministicReport schema

## Overview

\`DeterministicReport\` is the canonical contract returned by \`validateDeterministic()\` in \`@rulebound/engine\`. Every CLI output format (\`pretty\`, \`json\`, \`github\`, \`sarif\`, \`repair-json\`, \`pr-markdown\`), the MCP deterministic tools, and downstream consumers (dashboards, history snapshots, CI summaries) read from this shape. The types live in \`packages/engine/src/checks/\` and are re-exported through \`packages/engine/src/report-schema.ts\`, which is the single import path external code should use. The runtime guard \`validateDeterministicReport\` exists to defensively parse JSON snapshots loaded from disk or the wire.
...
- \`SCHEMA_VERSION\` is **doc-only** in v1.0. It is exported as a constant so callers that persist reports can stamp them (e.g. \`{ schemaVersion: "1.0.0", ...report }\`), but \`validateDeterministic()\` does **not** emit it.
- \`printJson\` / \`printSarif\` / \`printRepairJson\` output bytes are unchanged.
- Existing consumers that key on the current fields continue to work without modification.
...
Why: deterministic checks are only produced by rules that declare a \`checks:\` block. A rule with zero checks (advisory-only — typically a keyword or LLM matcher) produces zero \`CheckResult\` entries but still appears in \`ruleStatuses\` so consumers can render "this rule was considered but did not run deterministically." Therefore \`"ADVISORY"\` is a rule-level aggregate, not an individual check outcome.

## Field size warnings

\`evidence.stdout\` and \`evidence.stderr\` are emitted **verbatim** by analyzer and command runners. For verbose analyzers (PMD on a large monorepo, Semgrep with many rules, etc.) these strings can be hundreds of kilobytes to several megabytes. The engine does not truncate them at write time. Consumers that persist or transport reports (database columns, message queues, HTTP response bodies) should clamp / paginate / stream as appropriate. Some formatters (e.g. SARIF \`snippet\`) apply their own per-field truncation; the underlying \`DeterministicReport\` does not.

## Format coverage matrix

Which top-level / nested field each CLI \`--format\` reads. A checkmark means the formatter actually emits the field's content; blank means the format intentionally drops or aggregates the field away.
...
- **No field REMOVAL** in patch or minor releases. Removing a documented required field is a major-version break.
- **Adding optional fields is ALLOWED in minor.** Consumers must tolerate unknown fields.
- **Enum value additions are minor.** Adding a new \`DeterministicSource\` or a new \`confidence\` band is non-breaking.
- **Enum value removals are major.** Dropping \`"NOT_APPLICABLE"\` or \`"ADVISORY"\` would break consumers.
- **Unknown keys are ignored** by the runtime guard (\`validateDeterministicReport\`) for forward compatibility. Producers stamped with a newer minor \`SCHEMA_VERSION\` parse cleanly under an older consumer.
