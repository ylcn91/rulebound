...
const doc: DocPage = {
  ...
  title: "Quick Start",
  description:
    "Get Rulebound running deterministic checks on your repo in under 10 minutes — install, init a starter pack, run rulebound check, fix a deliberate failure, wire CI.",
  content: `## Quick Start

Goal: get Rulebound running deterministic checks on your repo in under 10 minutes.

This walkthrough installs the CLI, seeds deterministic rule packs, runs \`rulebound check\`, fixes one deliberate failure, and wires the result into CI. It does not require the server, dashboard, gateway, or hosted SaaS.
...
From the root of your repo, start with the \`starter\` pack — pure deterministic, no external analyzers required, no \`--allow-commands\`:
...
For demos or backwards-compatible examples, \`--examples\` still works but may emit analyzer warnings until the matching toolchain is installed:
...
Fix any \`fail\` items before adding command/analyzer rules that depend on local toolchains.
...
The check report is the authoritative pass/fail surface. Advisory commands such as \`rulebound validate\`, \`rulebound diff\`, and \`rulebound review\` are useful for planning and review, but they are not the deterministic CI gate.
...
That failure is the core Rulebound loop: the agent changed a protected file, but the diff did not include the required evidence. Add the missing migration and re-run the same command; the final pass/fail comes from the deterministic re-run, not from an LLM explanation.

#### Deliberate fail/fix loop

If your repo does not have a DB schema path, use a simple command/test evidence rule instead. Create \`.rulebound/rules/cli-command-needs-test.md\`:
...
See [GitHub Action](/docs/ci/github-action) for all inputs, the SARIF upload recipe, and the \`pr-markdown-summary\` / \`--allow-commands\` double-run trust boundary.
...
See [MCP Setup](/docs/mcp/setup). With MCP connected, the agent can call \`find_rules\` and advisory \`validate_plan\` early, then use authoritative \`run_deterministic_checks\`, \`check_diff\`, and \`get_repair_instructions\` before declaring the task complete. The CLI remains the final authority.

## What not to do

- Do not rely on \`rulebound review\`, \`rulebound validate\`, keyword matching, semantic matching, or LLM commentary as the final pass/fail. They are advisory unless you explicitly opt into \`--fail-on-advisory\`.
- Do not enable \`type: command\` or analyzer \`run:\` checks on untrusted rules. Use \`--allow-commands\` only in trusted repos and CI contexts.
- Do not route first-run users through the dashboard, server, or gateway. They are optional/advanced surfaces; \`rulebound check\` is the value moment.

## Next steps

- [Deterministic Checks](/docs/rules/deterministic-checks) — full check reference.
- [Analyzer Orchestration](/docs/recipes/orchestration) — wire PMD/Checkstyle/SpotBugs/ESLint/Semgrep.
- [Self-Healing Loop](/docs/workflows/self-healing) — repair loop contract.
- [Bugfix Workflow](/docs/workflows/bugfix-workflow) — bugfix boundary / postcondition / preservation.
`,
}
