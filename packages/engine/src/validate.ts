...
import type { Rule, Matcher, ValidateOptions, ValidationReport, ValidationResult } from "./types.js"
import type { PipelineResult } from "./types.js"

function buildRuleMap(rules: readonly Rule[]): ReadonlyMap<string, Rule> {
...
function convertToValidationResult(
  ...
  ruleMap: ReadonlyMap<string, Rule>
...
function computeSummary(results: readonly ValidationResult[]) {
  ...
  for (const result of results) {
    switch (result.status) {
      ...
      case "VIOLATED": violated += 1; break
      case "NOT_COVERED": notCovered += 1; break
    }
  }
...
function determineStatus(results: readonly ValidationResult[]): "PASSED" | "PASSED_WITH_WARNINGS" | "FAILED" {
  const hasMustViolation = results.some((r) => r.status === "VIOLATED" && r.modality === "must")
  if (hasMustViolation) return "FAILED"

  const hasWarnings = results.some((r) => r.status === "VIOLATED" || r.status === "NOT_COVERED")
...
export async function validate(options: ValidateOptions): Promise<ValidationReport> {
  ...
  if (useLlm) {
    try {
      ...
      matchers.push(new LLMMatcher({ provider: llmProvider ?? "anthropic", model: llmModel }))
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      throw new Error(`Failed to load LLM matcher: ${message}. Install AI packages: pnpm add ai @ai-sdk/anthropic`)
    }
  }
