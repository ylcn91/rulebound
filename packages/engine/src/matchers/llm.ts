...
function buildPrompt(plan: string, rule: Rule): {
...
type ModelProvider = (...args: any[]) => any

async function getModel(config: LLMConfig): Promise<ReturnType<ModelProvider>> {
...
async function validateRule(plan: string, rule: Rule, config: LLMConfig): Promise<MatchResult> {
  ...
  const { object } = await generateObject({ model, schema: resultSchema, system, prompt })
...
export class LLMMatcher implements Matcher {
  ...
  async match(context: MatcherContext): Promise<readonly MatchResult[]> {
    ...
    return processBatch(rules, CONCURRENCY_LIMIT, (rule) => validateRule(plan, rule, this.config))
  }
}
