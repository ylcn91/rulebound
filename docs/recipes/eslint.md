...
- `report_format: json` with `analyzer: eslint` reads ESLint's native
  array-shape JSON output directly (`[{filePath, messages:[{ruleId,
  severity, line, message}]}]`). No formatter plugin needed.
- If you prefer SARIF (cleaner per-rule metadata, ingestable by GitHub
  code-scanning), switch to `report_format: sarif` with
