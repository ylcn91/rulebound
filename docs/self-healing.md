...
An expired waiver re-enters `failures[]`; agents must fix it or ask the owner to
renew the waiver explicitly.
...
See [mcp-setup.md](mcp-setup.md) for tool wiring.
...
- Scenario evidence is planned. Once implemented, repairs must re-run the same
  deterministic scenario report/check; an LLM explanation of the scenario is
  never enough.
