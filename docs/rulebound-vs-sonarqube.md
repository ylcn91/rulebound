# Rulebound vs SonarQube

Short answer: they are different categories. Run both side by side.

For PR-review-bot comparisons, see
[Rulebound vs CodeRabbit](rulebound-vs-coderabbit.md).
...
- It will not become a full sandbox/twin provider. Future scenario evidence can
  require deterministic reports from external sandboxes or API twins; those
  systems provide the environment, Rulebound consumes the evidence.
