...
Runtime waivers live in `.rulebound/waivers.yaml` and must include `reason`,
`owner`, and `expires`. They downgrade a matching deterministic violation from
blocking to advisory without hiding it. Expired or malformed waivers fail closed.
See [waivers.md](waivers.md) for the authoritative schema.
