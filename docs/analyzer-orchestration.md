...
The CLI ships three opt-in analyzer packs. They install the rules but you
must install the underlying tools yourself and run with `--allow-commands`:
...
These are separate from the `starter` / `deterministic` / `agent-workflow`
packs, which do not pull in analyzer rules.
...
- [ESLint](./recipes/eslint.md)
- [TypeScript (`tsc --noEmit`)](./recipes/typescript-tsc.md)
- [Semgrep](./recipes/semgrep.md)
- [gitleaks](./recipes/gitleaks.md)
- [PMD](./recipes/pmd.md)
- [Checkstyle](./recipes/checkstyle.md)
- [SpotBugs](./recipes/spotbugs.md)
- [JUnit / Surefire / pytest XML](./recipes/junit.md)
...
A worked Java example lives in `examples/java-spring-demo/`.
